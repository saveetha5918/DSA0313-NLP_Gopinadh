...
def predict(grammar, chart, column, item):
...
def scan(tokens, chart, column, item):
    if column < len(tokens) and item.dot_position < len(item.production[1]) and \
